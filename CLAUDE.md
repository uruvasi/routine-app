...
### 2026-03-28 — セッション8: ルーティンタイマー タスクリストのスクロール改善

1. **タスクリストのスクロール対応** (v1.0.0)
   - RoutineTimer 実行画面で、タスク数が多い場合にタスクリスト部分のみ独立してスクロール可能に
   - 外側コンテナを `overflow-y-auto` → `overflow-hidden` に変更
   - CircularTimer・コントロール・変更/編集ボタンに `flex-shrink-0` を追加し常時表示を保証
   - タスクリストを `flex-1 min-h-0 overflow-y-auto` のラッパーで囲み、残スペース全体を使ってスクロール
   - `useRef` + `useEffect` でアクティブタスクへ自動スクロール（`scrollIntoView block:'nearest'`）

2. **バージョン 0.10.0 → 1.0.0**
...
- [x] ルーティン実行画面のタスクリスト独立スクロール + アクティブタスクへ自動スクロール
